...
## Create shapefile of station data
```{r}
...
# zoomed out
EastCoast <- ggplot() +
  #geom_sf(data = GB_sf, fill = NA, colour = "black", linewidth = 0.3, linetype = "dashed") +
  geom_sf(data = mass_sf, fill = "darkgrey", colour = "black") +
  geom_rect(aes(xmin = -70.5, xmax = -66, ymin = 40, ymax = 42.5),
            colour = "#08519C", fill = alpha("grey", 0)) +
  coord_sf(xlim = c(-75, -62), ylim = c(38, 45), crs = st_crs(4326)) +
  scale_x_continuous(breaks = c(-74, -70, -66, -62)) +
...
library(marmap)
GB_bathy <- getNOAA.bathy(lon1 = -71, lon2 = -65,
lat1 = 39, lat2 = 43, resolution = 1)
...
Georges <- ggplot() +
  ...
  scale_fill_manual(values = c("#08306B", "#084184", "#08519C", "#1561A9", "#2171B5", "#3282BE", "#4292C6", "#57A0CE", "#6BAED6", "#85BCDC", "#9ECAE1", "#B2D3E8", "#C6DBEF"), na.value = "#C6DBEF") +
  geom_sf(data = GB_sf, fill = NA, colour = "black", linewidth = 0.4, linetype = "dashed") +
  geom_point(data = station_sf[(station_sf$Station != 9 &
                                station_sf$Station != 43 &
                                station_sf$Station != 91), ],
             aes(x = Long, y = Lat), shape = 1) +
  geom_point(data = station_sf[station_sf$Station == 21, ], aes(x = Long, y = Lat),
             size = 3.5, shape = 17, stroke = 1.25) +
  geom_point(data = station_sf[station_sf$Station == 34, ], aes(x = Long, y = Lat),
             size = 3.5, shape = 17, stroke = 1.25) +
  geom_point(data = station_sf[station_sf$Station == 53, ], aes(x = Long, y = Lat),
             size = 3.5, shape = 17, stroke = 1.25) +
  geom_point(data = station_sf[station_sf$Station == 9, ], aes(x = Long, y = Lat),
             size = 3.5, shape = 2, stroke = 1.25) +
  geom_point(data = station_sf[station_sf$Station == 43, ], aes(x = Long, y = Lat),
             size = 3.5, shape = 2, stroke = 1.25) +
  geom_point(data = station_sf[station_sf$Station == 91, ], aes(x = Long, y = Lat),
             size = 3.5, shape = 2, stroke = 1.25) +
  geom_sf(data = mass_sf, fill = "darkgrey", colour = "black") +
...
ggdraw(Georges) +
  draw_plot(EastCoast, x = 0.22, y = 0.715, width = 0.3, height = 0.26)
```
