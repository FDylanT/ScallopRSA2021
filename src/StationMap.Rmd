...
borders <- Rgshhs("/Library/Frameworks/R.framework/Versions/4.2-arm64/Resources/library/PBSmapping/gshhg-bin-2.3.7/wdb_borders_f.b",
                xlim = c(286.5, 296.5), ylim = c(38, 44))
borders_sf <- as(borders2$SP, "SpatialLines") %>%
  st_as_sf() %>%
  st_transform(crs = 26919)

ggplot() +
  geom_sf(data = mass_sf, fill = "darkgrey", colour = NA) +
  geom_sf(data = borders_sf, colour = "black") +
  coord_sf(xlim = c(104812.98, 925854.69),
           ylim = c(4215380.56, 5108772.44),
           crs = st_crs(26919))
```
...
EastCoast <- ggplot() +
  ...
  geom_sf(data = mass_sf, fill = "darkgrey", colour = "black", linewidth = 0.05) +
  geom_sf(data = borders_sf, colour = "black", linewidth = 0.1) +
  geom_rect(aes(xmin = -70.5, xmax = -66, ymin = 40, ymax = 42.5),
            colour = "#08306B", fill = alpha("grey", 0), linewidth = 0.5) +
...
ggdraw(Georges) +
  draw_plot(EastCoast, x = 0.72, y = 0, width = 0.32, height = 0.32)
```
