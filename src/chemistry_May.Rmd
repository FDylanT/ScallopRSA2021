...
mayCarb <- carb(flag = 15,                               # 15 = ALK & DIC given
                var1 = may$TA[!is.na(may$Temp)] / 10^6,  # in mol/kg
                var2 = may$DIC[!is.na(may$Temp)] / 10^6, # in mol/kg
                S = may$Salinity[!is.na(may$Temp)],
                T = may$Temp[!is.na(may$Temp)],          # in deg C
                Patm = may$Pressure[!is.na(may$Temp)] * 0.098692326671601,
                                                         # in atm
                P = may$SeaPressure[!is.na(may$Temp)] / 10,   # in bar
...
for(i in c(1:10, 13:22, 25:30, 33:42, 45:58, 61:160, 163:220, 223:226)) {
  r <- 1
...
## Calculate contributions from each EM (triangles, not quadrangle)
```{r}
may$a1 <- NA
may$a2 <- NA
may$a3 <- NA
...
for(i in 1:nrow(may)) {
  if(may$Cast[i] == "Surface" &
     !is.na(may$MLD_frac[i])) {
    if(abs(may$d[i]) < 63) {
      may$DICf[i] = -1 * may$flux[i] / may$MLD[i] / may$Density[i] * may$d[i]
    } else {
...
## Write model
```{r}
model_filename <- "./bottom_model_may4.txt"
resid_err <- FALSE # because n=1 per Station
process_err <- TRUE
write_JAGS_model(model_filename, resid_err, process_err, mix, source)
...
run <- list(chainLength = 3000000, burn = 1500000, thin = 500, chains = 3,
            calcDIC = TRUE)
jags.2m4 <- run_model(run, mix, source, discr, model_filename, alpha.prior = 1)
```
...
diag <- output_diagnostics(jags.2m4, mix, source, output_options2m)
df.stats <- output_stats(jags.2m4, mix, source, output_options2m)
