...
```{r}
MLD <- read.csv("./data/CTD/may_MLD.csv") ***SITUATION***
...
mayResTime <- 60   # going with 60 across the board for now
may$DICf <- NA
...
for(i in 1:nrow(may)) {
  if(may$Cast[i] == "Surface") {
       may$DOn[i] = a1*DO1 + a2*DO2 + a3*DO3
       may$dDO[i] = may$DO[i] - may$DOn[i]   # observed - expected
            ...
            may$Station[i] != "093" &
            may$Station[i] != "104") {
       may$DOn[i] = a4*DO4 + a5*DO5 + a6*DO6
       may$dDO[i] = may$DO[i] - may$DOn[i]   # observed - expected
    } else {
      may$DOn[i] = a5*DO5 + a7*DO7
...
may$prDIC <- may$dDO * -106 / 138

# dDO umol/kg * 17 (TA) / 138 (O2) = prod/resp TA signal
may$prTA <- may$dDO * 17 / 138
```
...
may$DIC.other <- may$DIC - may$DICn - may$prDIC - may$DICf
may$TA.other = may$TA - may$TAn - may$prTA
```
