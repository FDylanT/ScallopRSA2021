...
may <- read.csv("./data/MayCruiseData.csv") %>%
  ...
  filter(!(Station == "112" & Cast == "Bottom"))
```
...
# 11:12, 23:24, 31:32, 43:44, 59:60, 161:162, 221:222, 227:228
...
for(i in 1:nrow(may)) {
  if(may$Cast[i] == "Bottom" &
     !is.na(may$dDIC[i]) &
     abs(may$dDIC[i]) > median(may$sdDICn, na.rm = T) &
     abs(may$dDO[i]) > median(may$sdDOn, na.rm = T)) {
...
may %>%
  filter(Cast == "Bottom" &
         Station != "037") %>%
ggplot() +
  geom_point(aes(x = OmegaAragonite, y = ratio, shape = Cast, col = "May")) +
  ylab("Ratio of TA excess : DIC excess") +
  scale_shape_manual(values = c(16, 1),
                     guide = guide_legend(reverse = TRUE)) +
  scale_colour_manual(values = "chartreuse3",
