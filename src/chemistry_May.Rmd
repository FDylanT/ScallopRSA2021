...
may <- read.csv("./data/MayCruiseData.csv") %>%
  rename(Lat = Latitude_degrees_start,
    ...
    select(Date, Station, Cast, Lat, Long,
         ...
         DIC, TA, DO) %>%
  mutate(Station = str_pad(Station, 3, pad = "0")) # add leading zeros to stations
```
...
```{r}
summ1m <- read.table("data/mixing-model/summary_statistics1m.txt", skip = 8) %>%
  select(-V1) %>%
...
```{r}
summ2m <- read.table("data/mixing-model/summary_statistics2m.txt", skip = 8) %>%
  dplyr::select(-V1) %>%
  separate(V2, into = c("Station", "EM")) %>%
  rename(Mean = V3, SD = V4)
...
# first satellite
satA <- nc_open("./data/MetOp-winds/KNMI-GLO-WIND_L3-OBS_METOP-A_ASCAT_25_ASC_V2_may.nc")
...
# second satellite
satB <- nc_open("./data/MetOp-winds/KNMI-GLO-WIND_L3-OBS_METOP-B_ASCAT_25_ASC_V2_may.nc")
...
# third satellite
satC <- nc_open("./data/MetOp-winds/KNMI-GLO-WIND_L3-OBS_METOP-C_ASCAT_25_ASC_V2_may.nc")
...
## OOI data
pCO2_data <- read.csv("./data/OOI-pCO2/MayCruiseCNSM.csv") %>%
...
# first is present-day eq DIC
may_co2_atm <- carb(24, pCO2a, may$TA[!is.na(may$Temp)] / 10^6,                 # in mol/kg
                    may$Salinity[!is.na(may$Temp)],
                    may$Temp[!is.na(may$Temp)],
                    may$Pressure[!is.na(may$Temp)] * 0.098692326671601,         # in atm
                    may$SeaPressure[!is.na(may$Temp)] / 10,                     # in bar
                    k1k2 = "l", ks = "d", b = "l10")
may_co2_atm_fut <- carb(24, 800, may$TA[!is.na(may$Temp)] / 10^6,               # in mol/kg
                    may$Salinity[!is.na(may$Temp)],
                    may$Temp[!is.na(may$Temp)],
                    may$Pressure[!is.na(may$Temp)] * 0.098692326671601,         # in atm
                    may$SeaPressure[!is.na(may$Temp)] / 10,                     # in bar
                    k1k2 = "l", ks = "d", b = "l10")
may_co2_atm_fut2050 <- carb(24, 550, may$TA[!is.na(may$Temp)] / 10^6,           # in mol/kg
                    may$Salinity[!is.na(may$Temp)],
                    may$Temp[!is.na(may$Temp)],
                    may$Pressure[!is.na(may$Temp)] * 0.098692326671601,         # in atm
                    may$SeaPressure[!is.na(may$Temp)] / 10,                     # in bar
...
may$co2_pres_dic <- NA
may$co2_future_dic <- NA
may$co2_future_dic2050 <- NA

r <- 1
for(i in 1:nrow(may)) {
  if(!is.na(may$Temp[i])) {
    may$co2_pres_dic[i] <- may_co2_atm$DIC[r]
    may$co2_future_dic[i] <- may_co2_atm_fut$DIC[r]
    may$co2_future_dic2050[i] <- may_co2_atm_fut2050$DIC[r]
    r <- r + 1
  }
}

may$deltaDIC <- may$co2_future_dic - may$co2_pres_dic   # delta specific to eq DIC difference
may$deltaDIC2050 <- may$co2_future_dic2050 - may$co2_pres_dic
may$DICfut <- (may$DIC / 10^6) + may$deltaDIC           # in umol/kg
may$DICfut2050 <- (may$DIC / 10^6) + may$deltaDIC2050   # in umol/kg

may_co2_fut <- carb(15, may$TA[!is.na(may$Temp) & !is.na(may$DIC)] / 10^6,                        # in mol/kg
                    may$DICfut[!is.na(may$Temp) & !is.na(may$DIC)],                               # in umol/kg
                    may$Salinity[!is.na(may$Temp) & !is.na(may$DIC)],
                    may$Temp[!is.na(may$Temp) & !is.na(may$DIC)],
                    may$Pressure[!is.na(may$Temp) & !is.na(may$DIC)] * 0.098692326671601,         # in atm
                    may$SeaPressure[!is.na(may$Temp) & !is.na(may$DIC)] / 10,                     # in bar
                    k1k2 = "l", ks = "d", b = "l10")
may_co2_fut2050 <- carb(15, may$TA[!is.na(may$Temp) & !is.na(may$DIC)] / 10^6,                    # in mol/kg
                    may$DICfut2050[!is.na(may$Temp) & !is.na(may$DIC)],                           # in umol/kg
                    may$Salinity[!is.na(may$Temp) & !is.na(may$DIC)],
                    may$Temp[!is.na(may$Temp) & !is.na(may$DIC)],
                    may$Pressure[!is.na(may$Temp) & !is.na(may$DIC)] * 0.098692326671601,         # in atm
                    may$SeaPressure[!is.na(may$Temp) & !is.na(may$DIC)] / 10,                     # in bar
...
may$omega_fut <- NA
may$omega_fut2050 <- NA

r <- 1
for(i in 1:nrow(may)) {
  if(!is.na(may$Temp[i]) & !is.na(may$DIC[i])) {
    may$omega_fut[i] <- may_co2_fut$OmegaAragonite[r]
    may$omega_fut2050[i] <- may_co2_fut2050$OmegaAragonite[r]
    r <- r + 1
  }
}
```
...
read.csv("./data/MayCruiseData.csv") %>%
  ...
  filter(Cast == "Surface",
         ...
         Station != 47 &
         Station != 2) %>%
  write_csv("./mixing-model/MayCruiseData_surface.csv")

# load mixture data
mix <- load_mix_data(filename = "./mixing-model/MayCruiseData_surface.csv", 
...
read.csv("./data/MayCruiseData.csv") %>%
  ...
  filter(Cast == "Bottom",
         ...
         Station != 4 &
         Station != 113) %>%
  write_csv("./mixing-model/MayCruiseData_bottom.csv")

# load mixture data
mix <- load_mix_data(filename = "./mixing-model/MayCruiseData_bottom.csv", 
                     iso_names = c("Salinity", "Temp"),
                     factors = "Station",
                     fac_random = FALSE, # indiv. station is fixed effect
                     fac_nested = NULL,
