...
may <- read.csv("./data/MayCruiseData.csv") %>%
  rename(Lat = Latitude_degrees_start,
         ...
         Depth = Depth_m_CTD,
         Temp = Temperature) %>%
  select(Station, Cast, Lat, Long, Depth, Temp, Pressure, SeaPressure, Salinity, DIC, TA) %>%
...
may$Salinity[which(is.na(may$Salinity))] <- 32.9 # give dummy salinities
...
mayCarb <- carb(15, may$TA / 10^6, may$DIC / 10^6, S = may$Salinity, T = may$Temp, Patm = may$Pressure * 0.098692326671601, P = may$SeaPressure / 10, k1k2 = "l", ks = "d", b = "l10") # flag:15 = ALK & DIC   # k1k2:l = Lueker et al. (2000)   # kf = unspecified b/c low temps   # ks:d = Dickson (1990)   # b:l10 = Lee et al. (2010)
...
station_SPDF_m <- SpatialPointsDataFrame(coords = may[ , c("Long", "Lat")],
                                        data = may[ , c("Long", "Lat", "Station", "Cast", "Depth", "Temp", "Salinity", "DIC", "TA", "pH", "fCO2", "OmegaAragonite", "OmegaCalcite")],
...
bottom_sf_m_v <- st_intersection(st_cast(bottom_sf_m_v), st_union(st_buffer(GB_sf, dist = 500)))
bottom_sf_m_v <- st_sf(bottom_sf_m_v)
...
surface_sf_m_v <- st_intersection(st_cast(surface_sf_m_v), st_union(st_buffer(GB_sf, dist = 500)))
surface_sf_m_v <- st_sf(surface_sf_m_v)
