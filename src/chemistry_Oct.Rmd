...
```{r}
summ1 <- read.table("dylan.titmuss/summary_statistics1.txt", skip = 8) %>%
  dplyr::select(-V1) %>%
...
```{r}
summ3 <- read.table("dylan.titmuss/summary_statistics3.txt", skip = 8) %>%
  dplyr::select(-V1) %>%
  separate(V2, into = c("Station", "EM")) %>%
  rename(Mean = V3, SD = V4)
...
```{r}
summ2 <- read.table("dylan.titmuss/summary_statistics2.txt", skip = 8) %>%
  dplyr::select(-V1) %>%
  separate(V2, into = c("Station", "EM")) %>%
  rename(Mean = V3, SD = V4)
...
```{r}
# sample collection: Oct 5-10, 2021
