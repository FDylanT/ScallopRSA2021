...
# first satellite
satA <- nc_open("./data/winds/KNMI-GLO-WIND_L3-OBS_METOP-A_ASCAT_12_ASC_V2_1671822354110.nc")
...
lonlat <- as.matrix(expand.grid(lon, lat))   # 34810 x 2
wind_vec <- as.vector(wind_slice) # L:34810

wind_df_A <- data.frame(cbind(lonlat, wind_vec))
names(wind_df_A) <- c("lon", "lat", "wind_speed")

# second satellite
satB <- nc_open("./data/winds/KNMI-GLO-WIND_L3-OBS_METOP-B_ASCAT_12_ASC_V2_1672170409134.nc")
...
wind_df_B <- data.frame(cbind(lonlat, wind_vec))
names(wind_df_B) <- c("lon", "lat", "wind_speed")

# third satellite
satC <- nc_open("./data/winds/KNMI-GLO-WIND_L3-OBS_METOP-C_ASCAT_12_ASC_V2_1671823208351.nc")
...
wind_df_C <- data.frame(cbind(lonlat, wind_vec))
names(wind_df_C) <- c("lon", "lat", "wind_speed")

# merge dataframes
wind_df <- bind_rows(wind_df_A, wind_df_B, wind_df_B) %>%
  group_by(lon, lat) %>%
...
ggplot() +
  geom_point(data = wind_df, aes(x = lon, y = lat, col = mean_wind), size = 7, shape = 15) +
  scale_color_viridis() +
  #xlim(-70.5, -66) +
  #ylim(40, 42.5) +
  theme_bw()
