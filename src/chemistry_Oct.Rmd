...
EM6 <- which(oct$Station == "053" & oct$Cast == "Bottom")
...
for(i in 1:nrow(oct)) {
  if(oct$Cast[i] == "Surface" &
    ...
    oct$EM3sd[i] <- summ1$SD[summ1$EM == "EM3" &
                           summ1$Station == as.integer(oct$Station[i])]
    oct$sum[i] <- oct$EM1[i] + oct$EM2[i] + oct$EM3[i]
  }
}
...
EMs4 <- oct$Salinity[oct$Cast == "Bottom" &
                     oct$Station != "022" & oct$Station != "023" &
                     oct$Station != "104" & 
                     oct$Salinity >= S4 - 0.3 & oct$Salinity <= S4 + 0.3 &
...
EMs5 <- oct$Salinity[oct$Cast == "Bottom" &
                     oct$Station != "022" & oct$Station != "023" &
                     oct$Station != "104" & 
                     oct$Salinity >= S5 - 0.3 & oct$Salinity <= S5 + 0.3 &
...
EMs6 <- oct$Salinity[oct$Cast == "Bottom" &
                     oct$Station != "022" & oct$Station != "023" &
                     oct$Station != "104" & 
                     oct$Salinity >= S6 - 0.3 & oct$Salinity <= S6 + 0.3 &
...
calc_area(source = source, mix = mix, discr = discr) # 1411.205
