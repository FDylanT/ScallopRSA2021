...
EMs7 <- oct$Salinity[oct$Cast == "Surface" &
                     oct$Station != "022" & oct$Station != "023" &
                     oct$Station != "104" & 
...
for(i in 1:nrow(oct)) {
  if(oct$Cast[i] == "Bottom" &
    ...
    oct$TAcorr[i] <- oct$TA[i] - (oct$AOU[i] * -17 / -138)
  }
}
...
for(i in 1:nrow(oct)) {
  if(oct$Cast[i] == "Surface" &
     !is.na(oct$MLD_frac[i])) {
    if(abs(oct$d[i]) < 63) {
      oct$DICf[i] = -1 * oct$flux[i] / oct$MLD[i] / oct$Density[i] * oct$d[i]
    } else {
...
for(i in 1:nrow(oct)) {
  if(!is.na(oct$MLD_frac[i]) &
     ...
     (oct$Cast[i] == "Bottom" &
     oct$MLD_frac[i] > 0.9))) {
    if(abs(oct$d.O2[i]) < 63) {
      oct$DOf[i] = -1 * oct$flux.O2[i] / oct$MLD[i] / oct$Density[i] * oct$d.O2[i]
    } else {
      oct$DOf[i] = -1 * oct$flux.O2[i] / oct$MLD[i] / oct$Density[i] * octResTime
    }
  }
}
