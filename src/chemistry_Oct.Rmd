...
# second satellite
satB <- nc_open("./data/winds/KNMI-GLO-WIND_L3-OBS_METOP-C_ASCAT_12_ASC_V2_1671823208351.nc")
...
# merge dataframes
wind_df <- bind_rows(wind_df_A, wind_df_B) %>%
  group_by(lon, lat) %>%
...
ggplot(wind_df, aes(x = lon, y = lat, col = mean_wind)) +
  geom_point() +
  scale_color_viridis() +
  xlim(-71, -66) +
  ylim(40, 43) +
