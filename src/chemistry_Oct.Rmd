...
U10 <- mean(octBuoy$WSPD)       ##### this is not corrected to 10m
...
for(i in 1:nrow(oct)) {
  if(oct$Cast[i] == "Surface") {
    ...
    oct$K0[i] = oct$K.prime[i] * oct$X[i] * (P - oct$pH2O[i]) / oct$fCO2[i]
  }
}
...
for(i in 1:nrow(oct)) {
  if(oct$Cast[i] == "Surface") {
    ...
    oct$flux[i] = k * K0 * (oct$pCO2[i] - pCO2a) # umol/(m^2*day)
  }
}
...
for(i in 1:nrow(oct)) {
  if(oct$Cast[i] == "Surface") {
    oct$dC[i] = flux / oct$MLD[i] / oct$Density[i] * octResTime
  }
}
```
