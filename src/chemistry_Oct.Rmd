...
EM2 <- which(oct$Station == "113" & oct$Cast == "Surface")
EM3 <- which(oct$Station == "030" & oct$Cast == "Surface")

EM7 <- which(oct$Station == "005" & oct$Cast == "Surface")

# bottom EMs
EM6 <- which(oct$Station == "021" & oct$Cast == "Bottom")
...
for(i in 1:nrow(oct)) {
  if(oct$Cast[i] == "Surface" &
     ...
     oct$Station[i] != "005") {
       ...
       a7 = (oct$Salinity[i] - S1) / (S7 - S1)
       a1 = 1 - a7
       oct$a1[i] = a1
       oct$a7[i] = a7
       oct$DICn[i] = a1*DIC1 + a7*DIC7
       oct$dDIC[i] = oct$DIC[i] - oct$DICn[i]   # observed - expected
       oct$TAn[i] = a1*TA1 + a7*TA7
       oct$dTA[i] = oct$TA[i] - oct$TAn[i]   # observed - expected
     }
}
```
...
for(i in 1:nrow(oct)) {
  if(oct$Cast[i] == "Surface") {
    ...
    d.O2 = (oct$DO_L[i] - oct$O2sol_L[i]) * 10^3   # umol/L --> umol/m^3
    oct$flux.O2[i] = k * d.O2   # umol/(m^2*d)
  }
}
...
for(i in 1:nrow(oct)) {
  if(oct$Cast[i] == "Surface" |
     ...
     !is.na(oct$MLD_frac[i]) &
     oct$MLD_frac[i] > 0.9)) {
    oct$O2f[i] = -1 * oct$flux.O2[i] / oct$MLD[i] / oct$Density[i] * octResTime
  }
}
