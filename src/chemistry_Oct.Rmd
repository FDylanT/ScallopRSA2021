...
# k (cm/hr) = 0.251 * U^2 * (Sc/660)^-0.5
  # U (m/s) = average of neutral stability winds at 10-m height
      # ****need NDBC data corr. to 10m****
      # average 30 days around cruise

# NDBC buoys:
  # 44008: May & October 2021
  # 44011: October 2021
  # 44024: October 2021
...
for(i in 1:nrow(oct)) {
  if(oct$Cast[i] == "Surface") {
    ...
    oct$pH2O[i] = exp(24.4543 - 67.4509*(100/T) - 4.8489*log(T/100) - 0.000544*S)   # atm?
    ###### ^ check this equation; also what are units?
  }
}
...
P <- mean(octBuoy$PRES)   # hPa
oct$K0 <- NA

for(i in 1:nrow(oct)) {
  if(oct$Cast[i] == "Surface") {
    oct$X[i] = p2xCO2(oct$Salinity[i], oct$Temp[i], Patm = P/1013.25, oct$pCO2[i])   # ppm
    oct$X2[i] = oct$pCO2[i] / (P/1013.25 - oct$pH2O[i])
    oct$K0[i] = oct$K.prime[i] * X * (P - oct$pH2O[i]) / oct$fCO2[i]
  }
}
...
for(i in 1:nrow(oct)) {
  if(oct$Cast[i] == "Surface") {
    k = oct$k[i] / 100 * 24          # cm/hr --> m/day
    K0 = oct$K0[i] * 10^6 / 10^6 * 10^3    # mol/(L*atm) --> umol/(m^3*uatm)
    flux = k * K0 * (oct$pCO2[i] - pCO2a)
    oct$flux[i] = flux / oct$MLD[i] / oct$Density[i] * res.time
  }
}

# final unit situation:
# m/day * umol/(m^3*uatm) * uatm = m*umol*uatm / day*m^3*uatm  = umol/(day*m^2)
# umol/(day*m^2) * 1/[MLD]m * m^3/[rho]kg * [res.time]days = umol/kg
