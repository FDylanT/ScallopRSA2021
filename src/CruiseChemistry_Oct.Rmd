...
oct_full <- read.csv("./data/OctCruiseData.csv") %>%
  rename(Lat = Latitude_degrees_start,
         ...
         Depth = Depth_m_CTD,
         Temp = Temperature) %>%
  select(Station, Cast, Filtered, Lat, Long, Depth, Temp, Salinity, DIC, TA) %>%
...
```{r}
# get Patm and SeaPressure from MATLAB sometime
# replace Depth with actual pressure :))

oct$Salinity[which(is.na(oct$Salinity))] <- 32.9 # give dummy salinities

octCarb <- carb(15, oct$TA / 10^6, oct$DIC / 10^6, S = oct$Salinity, T = oct$Temp, Patm = 1, P = oct$Depth / 10, k1k2 = "l", ks = "d", b = "l10") # flag:15 = ALK & DIC   # k1k2:l = Lueker et al. (2000)   # kf = unspecified b/c low temps   # ks:d = Dickson (1990)   # b:l10 = Lee et al. (2010)

# calculate density
oct$Density <- gsw_rho(oct$Salinity, oct$Temp, oct$Depth)
```

## Combine oct and carb data
```{r}
oct <- cbind(oct,
             pH = octCarb$pH,
             fCO2 = octCarb$fCO2,
...
### maybe check 009 bottom DIC -- done
...
station_SPDF <- SpatialPointsDataFrame(coords = oct[ , c("Long", "Lat")],
                                        data = oct[ , c("Long", "Lat", "Station", "Cast", "Depth", "Temp", "Salinity", "Density", "DIC", "TA", "pH", "fCO2", "OmegaAragonite", "OmegaCalcite")],
