...
# plot(Pressure_dbar ~ Conductivity_mS.cm, data = octCTD, ylim = c(120, 0), main = "October CTD Casts")

ggplot(octCTD, aes(x = Salinity_PSU, y = Pressure_dbar)) +
  geom_point(na.rm = TRUE) +
  ylim(c(120, 0)) +
...
#### Save decimal second timestamps in case needed
```{r}
oct1Times <- as.character(oct1$Time)
write.csv(oct1Times, "data/oct1Times.txt")

oct2Times <- as.character(oct2$Time)
write.csv(oct2Times, "data/oct2Times.txt")

oct3Times <- as.character(oct3$Time)
write.csv(oct3Times, "data/oct3Times.txt")

# Call these vectors
oct1Times_read <- read.csv("data/CTD_raw/oct1Times.txt")
oct2Times_read <- read.csv("data/CTD_raw/oct2Times.txt")
oct3Times_read <- read.csv("data/CTD_raw/oct3Times.txt")
```
