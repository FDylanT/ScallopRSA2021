...
ggplot(Placo, aes(x = Site, y = ShellHt_mm, col = Month)) +
  ...
  #labs(title = "Scallop Shell Heights by Site") +
  theme_bw() +
  theme(panel.grid.major.x = element_blank())

ggplot(mayPlaco, aes(x = Site, y = ShellWt_g)) +
  geom_boxplot() +
  labs(y = "Shell Weight (g)", title = "Scallop Shell Weights by Site") +
  theme_bw()

ggplot(mayPlaco, aes(x = Site, y = ShellWt_g / ShellHt_mm)) +
  geom_boxplot() +
  labs(y = "Ratio of Shell Weight to Shell Height (g/mm)", title = "Linear Density of Scallop Shells") +
  theme_bw()
```

## Create shapefile of Placo data
```{r}
...
# add scallop bio data
station_sf <- station_sf %>%
  filter(Tow == "Y")

for(i in 1:nrow(station_sf)) {
  station = station_sf$Station[i]
  ShellHt[i] = mean(Placo$ShellHt_mm[Placo$Site == station])
}
```

## Create maps
```{r}
# using code from StationMap.Rmd
...
ggplot() +
  ...
  geom_point(data = station_sf[station_sf$Tow == "Y", ], aes(x = Long, y = Lat), shape = 17) +
