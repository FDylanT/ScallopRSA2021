...
# DICas
min(surface_sf2$DICas) # -68.81886
max(surface_sf2$DICas) # 40.44953
```
...
ggplot() +
  ...
  geom_sf(data = mass_sf, fill = "darkgrey", colour = NA) +
  scale_fill_distiller(limits = c(-13, 13), palette = "BrBG",
                       breaks = c(-12, 0, 12),
                       aesthetics = c("fill", "colour"),
                       name = "CO2 flux\n(mmol/m^2/d)") +
...
ggplot() +
  geom_sf(data = surface_sf_v2, aes(fill = DICas), size = 0.25, colour = NA) +
  geom_sf(data = surface_sf3, aes(col = DICas), size = 8, shape = 18) +
  geom_sf(data = mass_sf, fill = "darkgrey", colour = NA) +
  scale_fill_distiller(limits = c(-25, 25), palette = "BrBG",
                       breaks = c(-20, 0, 20),
                       aesthetics = c("fill", "colour"),
                       name = "DIC[air-sea]\n(umol/kg)") +
  scale_y_continuous(breaks = c(40, 41, 42)) +
