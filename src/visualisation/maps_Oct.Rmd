...
# dDIC
min(bottom_sf2$dDIC) # -70.65599
min(surface_sf2$dDIC) # -14.49379
max(bottom_sf2$dDIC) # 54.72723
max(surface_sf2$dDIC) # 36.97239

# flux
min(bottom_sf2$flux, na.rm = T) # 2952.179
min(surface_sf2$flux)           # -16467
max(bottom_sf2$flux, na.rm = T) # 10375.19
max(surface_sf2$flux)           # 7820.752

# DICf
min(bottom_sf2$DICf, na.rm = T) # 
min(surface_sf2$DICf, na.rm = T) # -34.78258
max(bottom_sf2$DICf, na.rm = T) # 
max(surface_sf2$DICf, na.rm = T) # 77.47505

# dDO
min(bottom_sf2$dDO) # -27.66798
min(surface_sf2$dDO) # -3.232047
max(bottom_sf2$dDO) # 3.994369
max(surface_sf2$dDO) # 27.0712

# DICpr
min(bottom_sf2$DICpr) # -3.068139
min(surface_sf2$DICpr) # -20.79382
max(bottom_sf2$DICpr) # 21.25222
max(surface_sf2$DICpr) # 2.482587

# DIC.other
min(bottom_sf2$DIC.other) # -21.46247
min(surface_sf2$DIC.other) # -78.43769
max(bottom_sf2$DIC.other) # 73.69208
max(surface_sf2$DIC.other) # 65.39927
```
...
ggplot() +
  ...
  geom_sf(data = mass_sf, fill = "darkgrey", colour = NA) +
  scale_fill_distiller(limits = c(-18.5, 18.5), palette = "RdBu",
                       breaks = c(-12, 0, 12),
                       aesthetics = c("fill", "colour"),
                       name = "CO2 flux\n(mmol/m^2/d)") +
  scale_y_continuous(breaks = c(40, 41, 42)) +
...
ggplot() +
  ...
  geom_sf(data = mass_sf, fill = "darkgrey", colour = NA) +
  scale_fill_distiller(limits = c(-90, 90), palette = "RdBu",
                       aesthetics = c("fill", "colour")) +
  scale_y_continuous(breaks = c(40, 41, 42)) +
  coord_sf(xlim = c(371957.88, 746508.34), ylim = c(4428833.79, 4709654.16)) +
...
ggplot() +
  ...
  geom_sf(data = mass_sf, fill = "darkgrey", colour = NA) +
  scale_fill_distiller(limits = c(-22, 22), palette = "RdBu",
                       aesthetics = c("fill", "colour")) +
  scale_y_continuous(breaks = c(40, 41, 42)) +
  coord_sf(xlim = c(371957.88, 746508.34), ylim = c(4428833.79, 4709654.16)) +
...
```{r}
# TAn
min(bottom_sf2$TAn) # 2181.2
min(surface_sf2$TAn) # 2143.2
max(bottom_sf2$TAn) # 2355
max(surface_sf2$TAn) # 2281.1

# dTA
min(bottom_sf2$dTA) # -11.32524
min(surface_sf2$dTA) # -14.26346
max(bottom_sf2$dTA) # 53.52491
max(surface_sf2$dTA) # 31.87739

# TApr
min(bottom_sf2$TApr) # -3.408375
min(surface_sf2$TApr) # -0.3981507
max(bottom_sf2$TApr) # 0.49206
max(surface_sf2$TApr) # 3.334858

# TA.other
min(bottom_sf2$TA.other) # -11.42857
min(surface_sf2$TA.other) # -16.03003
max(bottom_sf2$TA.other) # 53.56942
max(surface_sf2$TA.other) # 28.7857
```

## Bottom plots
```{r}
ggplot() +
  geom_sf(data = bottom_sf_v2, aes(fill = TAn), size = 0.25, colour = NA) +
  geom_sf(data = bottom_sf3, aes(col = TAn), size = 8, shape = 18) +
  geom_sf(data = mass_sf, fill = "darkgrey", colour = NA) +
  scale_fill_viridis(limits = c(2140, 2360), option = "rocket",
                     aesthetics = c("fill", "colour")) +
  scale_y_continuous(breaks = c(40, 41, 42)) +
  coord_sf(xlim = c(371957.88, 746508.34), ylim = c(4428833.79, 4709654.16)) +
  labs(title = "Bottom TA_mixing [Oct]") +
  theme_bw()
...
```{r}
ggplot() +
  geom_sf(data = surface_sf_v2, aes(fill = TAn), size = 0.25, colour = NA) +
  geom_sf(data = surface_sf3, aes(col = TAn), size = 8, shape = 18) +
  geom_sf(data = mass_sf, fill = "darkgrey", colour = NA) +
  scale_fill_viridis(limits = c(2140, 2360), option = "rocket",
                     aesthetics = c("fill", "colour")) +
  scale_y_continuous(breaks = c(40, 41, 42)) +
  coord_sf(xlim = c(371957.88, 746508.34), ylim = c(4428833.79, 4709654.16)) +
  labs(title = "Surface TA_mixing [Oct]") +
  theme_bw()
...
### 10-panel derived parameter maps
```{r}
omega <- ggplot() +
...
plot_grid(
  plot_grid(
    may_flux, oct_flux + theme(legend.position = "none"),
    ncol = 2,
    align = "hv",
    labels = c("A", "B")),
  plot_grid(
...
plot_grid(
  plot_grid(
    odDIC,
    odDIC2 + theme(legend.position = "none"),
    oDICf,
...
## 6-panel relative TA contributions
```{r}
dTA <- ggplot() +
  geom_sf(data = surface_sf_v2, aes(fill = dTA), size = 0.25, colour = NA) +
  geom_sf(data = surface_sf3, aes(col = dTA), size = 8, shape = 18) +
