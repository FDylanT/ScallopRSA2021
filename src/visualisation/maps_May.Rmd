...
sort(bottom_sf_m2$pCO2)[2] # 310.9799
...
```{r}
# dDIC
  #min(bottom_sf_m2$dDIC) # -74.87484 - outlier
sort(bottom_sf_m2$dDIC)[2] # -15.78552
  #min(surface_sf_m2$dDIC) # -39.65087
sort(surface_sf_m2$dDIC)[2] # -32.92995
max(bottom_sf_m2$dDIC) # 32.64742
max(surface_sf_m2$dDIC) # 22.28002
...
# DICf
min(surface_sf_m2$DICf, na.rm = T) # -4.952685
max(surface_sf_m2$DICf, na.rm = T) # 51.62271

# dDO
min(bottom_sf_m2$dDO) # -25.05694
min(surface_sf_m2$dDO) # -9.447088
max(bottom_sf_m2$dDO) # 22.41824
  #max(surface_sf_m2$dDO) # 29.02208
sort(surface_sf_m2$dDO, decreasing = TRUE)[2] # 17.36457

# DICpr
min(bottom_sf_m2$DICpr) # -17.2198
min(surface_sf_m2$DICpr) # -22.29232
max(bottom_sf_m2$DICpr) # 19.24664
max(surface_sf_m2$DICpr) # 8.110144

# DIC.other
  #min(bottom_sf_m2$DIC.other) # -76.94278 - outlier
sort(bottom_sf_m2$DIC.other)[2] # -17.86817
min(surface_sf_m2$DIC.other) # -40.99212
max(bottom_sf_m2$DIC.other) # 32.78591
max(surface_sf_m2$DIC.other) # 21.49272
```
...
```{r}
# dTA
min(bottom_sf_m2$dTA) # -7.708125
min(surface_sf_m2$dTA) # -33.71277
max(bottom_sf_m2$dTA) # 29.85169
max(surface_sf_m2$dTA) # 23.55299

# TApr
min(bottom_sf_m2$TApr) # -3.170982
min(surface_sf_m2$TApr) # -1.333626
max(bottom_sf_m2$TApr) # 2.831245
max(surface_sf_m2$TApr) # 3.666205

# TA.other
min(bottom_sf_m2$TA.other) # -6.46663
min(surface_sf_m2$TA.other) # -33.49127
max(bottom_sf_m2$TA.other) # 30.70117
max(surface_sf_m2$TA.other) # 22.04645
```
...
pCO2.2 <- ggplot() +
  geom_sf(data = bottom_sf_m_v2, aes(fill = pCO2), size = 0.25, colour = NA) +
  geom_sf(data = bottom_sf_m3, aes(col = pCO2), size = 6, shape = 18) +
  geom_sf(data = mass_sf, fill = "darkgrey", colour = NA) +
  scale_fill_distiller(limits = c(179, 661),
                       palette = "PRGn",
                       aesthetics = c("fill", "colour"),
...
dDIC <- ggplot() +
  ...
  geom_sf(data = mass_sf, fill = "darkgrey", colour = NA) +
  scale_fill_distiller(limits = c(-33, 33), palette = "RdBu",
...
dDO <- ggplot() +
  ...
  geom_sf(data = mass_sf, fill = "darkgrey", colour = NA) +
  scale_fill_distiller(limits = c(-26, 26), palette = "RdBu",
...
### 8-panel relative DIC contribution maps
```{r}
dDIC <- ggplot() +
  geom_sf(data = surface_sf_m_v2, aes(fill = dDIC), size = 0.25, colour = NA) +
  geom_sf(data = surface_sf_m3, aes(col = dDIC), size = 8, shape = 18) +
  geom_sf(data = mass_sf, fill = "darkgrey", colour = NA) +
  scale_fill_distiller(limits = c(-41, 41), palette = "RdBu",
                       aesthetics = c("fill", "colour")) +
  scale_y_continuous(breaks = c(40, 41, 42)) +
  coord_sf(xlim = c(371957.88, 746508.34), ylim = c(4428833.79, 4709654.16)) +
  #labs(title = "Surface dDIC [May]") +
...
plot_grid(
  plot_grid(
    dDIC,
    dDIC2 + theme(legend.position = "none"),
    #DICf,
...
### 6-panel relative TA contribution maps
```{r}
dTA <- ggplot() +
  geom_sf(data = surface_sf_m_v2, aes(fill = dTA), size = 0.25, colour = NA) +
  geom_sf(data = surface_sf_m3, aes(col = dTA), size = 8, shape = 18) +
