...
## Prop-prop plots
```{r}
# Sal vs. Temp, coloured cast
ggplot(chem, aes(x = Salinity, y = Temp, shape = Cast, col = Cast)) +
  geom_point() +
  theme_bw()

# Sal vs. Temp, bottom, coloured depth
#plot <-
chem %>%
  filter(Cast == "Bottom") %>%
  ggplot(aes(x = Salinity, y = Temp, col = CTD_depth)) +
  geom_point() +
...
# Sal vs. Temp, surface, coloured bottom depth
chem %>%
  filter(Cast == "Surface") %>%
  ggplot(aes(x = Salinity, y = Temp, col = CTD_depth)) +
  geom_point(shape = 17) +
...
# Sal vs. Temp, coloured DIC
ggplot(chem, aes(x = Salinity, y = Temp, col = DIC_avg, shape = Cast)) +
  geom_point() +
  scale_colour_viridis(option = "rocket") +
  theme_bw()

# Sal vs. Temp, coloured TA
ggplot(chem, aes(x = Salinity, y = Temp, col = TA_avg, shape = Cast)) +
  geom_point() +
  scale_colour_viridis() +
  theme_bw()

# Sal vs. DIC, coloured cast
ggplot(chem, aes(x = Salinity, y = DIC_avg, shape = Cast, col = Cast)) +
  geom_point() +
  theme_bw()

plot(DIC_avg ~ Salinity, data = chem)
identify(chem$DIC_avg ~ chem$Salinity)

### check 112A DIC

# Sal vs. TA, coloured cast
ggplot(chem, aes(x = Salinity, y = TA_avg, shape = Cast, col = Cast)) +
  geom_point() +
  theme_bw()
...
ggplot(chem, aes(x = DIC_avg, y = TA_avg, shape = Cast, col = Cast)) +
  geom_point() +
  theme_bw()

plot(chem$TA_avg ~ chem$DIC_avg)
identify(chem$TA_avg ~ chem$DIC_avg)
...
bottom_sf1 <- station_sf %>%
  filter(Cast == "Bottom")

surface_sf1 <- station_sf %>%
  filter(Cast == "Surface")
```
...
### Check mapping
```{r}
ggplot(bottom_sf1, aes(x = Long, y = Lat)) +
...
bottom_sf <- bottom_sf1 %>%
  mutate(Long = ifelse(Station == "006", Long + 0.06, Long),
         Lat = ifelse(Station == "026", Lat + 0.02, Lat),
         Long = ifelse(Station == "089", Long - 0.04, Long))

surface_sf <- surface_sf1 %>%
  mutate(Long = ifelse(Station == "006", Long + 0.06, Long),
         Lat = ifelse(Station == "026", Lat + 0.02, Lat),
         Long = ifelse(Station == "089", Long - 0.04, Long))
```
...
ggplot() +
  geom_sf(data = bottom_sf_v3, aes(fill = DIC_avg), size = 0.25, colour = NA) +
  scale_fill_viridis() +
  coord_sf(xlim = c(371957.88, 746508.34), ylim = c(4428833.79, 4709654.16)) +
  labs(title = "Bottom DIC")

ggplot() +
  geom_sf(data = bottom_sf_v3, aes(fill = TA_avg), size = 0.25, colour = NA) +
  scale_fill_viridis() +
  coord_sf(xlim = c(371957.88, 746508.34), ylim = c(4428833.79, 4709654.16)) +
...
ggplot() +
  geom_sf(data = surface_sf_v2, aes(fill = DIC_avg), size = 0.25, colour = NA) +
  scale_fill_viridis() +
  coord_sf(xlim = c(371957.88, 746508.34), ylim = c(4428833.79, 4709654.16)) +
  labs(title = "Surface DIC")

ggplot() +
  geom_sf(data = surface_sf_v2, aes(fill = TA_avg), size = 0.25, colour = NA) +
  scale_fill_viridis() +
  coord_sf(xlim = c(371957.88, 746508.34), ylim = c(4428833.79, 4709654.16)) +
...
# idw for DIC
interp_DIC_B <- idw(DIC_avg ~ 1, bottom_sp, newdata = idw_grid, idp = 2) %>%
  as.data.frame()
interp_DIC_S <- idw(DIC_avg ~ 1, surface_sp, newdata = idw_grid, idp = 2) %>%
  as.data.frame()
...
# idw for TA
interp_TA_B <- idw(TA_avg ~ 1, bottom_sp, newdata = idw_grid, idp = 2) %>%
  as.data.frame()
interp_TA_S <- idw(TA_avg ~ 1, surface_sp, newdata = idw_grid, idp = 2) %>%
  as.data.frame()
...
bottomDIC <- map +
  geom_sf(data = bottom_sf1, aes(colour = DIC_avg), size = 5, inherit.aes = FALSE) +
  #geom_sf_text(data = bottom_sf1, aes(label = Station), nudge_x = 0, nudge_y = 0.09, size = 3.5, inherit.aes = FALSE) +
...
surfaceDIC <- map +
  geom_sf(data = surface_sf1, aes(colour = DIC_avg), size = 5, inherit.aes = FALSE) +
  #geom_sf_text(data = surface_sf1, aes(label = Station), nudge_x = 0, nudge_y = 0.09, size = 3.5, inherit.aes = FALSE) +
  scale_colour_viridis(limits = c(2000, 2150), option = "rocket") +
  #scale_size_continuous(limits = c(1950, 2150), range = c(2, 6)) +
  guides(colour = guide_legend(reverse = TRUE, title = "DIC (umol/kg)"), size = guide_legend(reverse = TRUE, title = "DIC (umol/kg)")) +
...
bottomTA <- map +
  geom_sf(data = bottom_sf1, aes(size = TA_avg, colour = TA_avg), inherit.aes = FALSE) +
  #geom_sf_text(data = bottom_sf1, aes(label = Station), nudge_x = 0, nudge_y = 0.09, size = 3.5, inherit.aes = FALSE) +
...
surfaceTA <- map +
  geom_sf(data = surface_sf1, aes(size = TA_avg, colour = TA_avg), inherit.aes = FALSE) +
  #geom_sf_text(data = surface_sf1, aes(label = Station), nudge_x = 0, nudge_y = 0.09, size = 3.5, inherit.aes = FALSE) +
  scale_colour_viridis(limits = c(2175, 2300), option = "rocket") +
  scale_size_continuous(limits = c(2175, 2300), range = c(2, 6)) +
  guides(colour = guide_legend(reverse = TRUE, title = "TA (umol/kg)"), size = guide_legend(reverse = TRUE, title = "TA (umol/kg)")) +
